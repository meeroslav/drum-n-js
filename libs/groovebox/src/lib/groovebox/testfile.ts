...
const dispatcher = _.extend({
  'EventKeys': {},
  register: function (eventHash) {
    for (const k in eventHash) {
...
const Sequencer = (function (A, S) {
  ...
  let tempo, tic, _initialized = false;
  let noteTime, startTime, ti, currentStep = 0;
  let isPlaying = false;
  let currentPattern = null, _currentPatternSequenceRaw;
  const channelStatus = {};
  ...
  function _parsePattern(pattern) {
    ...
    for (const k in pattern.sequence) {
      const pat = _parseLine(pattern.sequence[k]);
  ...
  const SequencerView = Backbone.View.extend({
    ...
    setPattern: function (pattern) {
      ...
      for (var k in currentPattern) {
        const $cel = this.$el.find('.channel[data-inst="' + k + '"]');
    ...
    render: function () {
      const data = (currentPattern) ? Object.keys(currentPattern) : [];
      const rawHTML = _template({ channels: data });
      this.$el.html(rawHTML);
      return this;
    },
    renderChannels: function () {
      this.$channelContainer = this.$el.find('.sequencer-channels');
      for (const k in this.channelViews) {
  ...
  const _channelTemplate = Handlebars.compile('\
  ...
  var ChannelView = Backbone.View.extend({
    ...
    render: function () {
      const rawHTML = _channelTemplate({
        id: this.channel,
    ...
    onNoteClick: function (e) {
      const tic = $(e.currentTarget).attr('data-tic');
...
const Transport = (function () {
  ...
  const TransportView = Backbone.View.extend({
    events: {
    ...
    onTempoChange: function (e) {
      const newTempo = $(e.currentTarget).val();
...
const Metronome = (function () {
  ...
  const MetronomeView = Backbone.View.extend({
    initialize: function (options) {
...
const PresetList = (function () {
  ...
  const PresetListView = Backbone.View.extend({
    events: {
      'click a': 'onPresetClick'
    },
    render: function () {
      const rawHTML = _template({ items: presets });
      this.$el.html(rawHTML);
      this.$items = this.$el.find('a');
      return this;
    },
    onPresetClick: function (e) {
      const id = $(e.currentTarget).attr('data-preset-id');
